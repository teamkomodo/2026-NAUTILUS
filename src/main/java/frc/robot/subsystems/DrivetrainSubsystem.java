...
public class DrivetrainSubsystem implements Subsystem {
    ...
    public Command joystickDriveCommand(DoubleSupplier xAxis, DoubleSupplier yAxis, DoubleSupplier rotAxis) {
        return Commands.run(() -> {
            ...
            xVelocity = MathUtil.applyDeadband(xAxis.getAsDouble(), 0.1) * -MAX_MODULE_VELOCITY
                    * (slowMode ? LINEAR_SLOW_MODE_MODIFIER : 1);
            yVelocity = MathUtil.applyDeadband(yAxis.getAsDouble(), 0.1) * -MAX_MODULE_VELOCITY
                    * (slowMode ? LINEAR_SLOW_MODE_MODIFIER : 1);
            angularVelocity = MathUtil.applyDeadband(rotAxis.getAsDouble(), 0.1) * MAX_ANGULAR_VELOCITY
